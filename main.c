...
{
	boolean result = (Det_ModuleId == ModuleId) && (Det_InstanceId == InstanceId) && (Det_ApiId == ApiId) &&
					 (Det_ErrorId == ErrorId);
	Det_ModuleId = 0;
	Det_InstanceId = 0;
	Det_ApiId = 0;
...
void TestCase1(void)
{
	...
	Dio_ReadChannel(DioConf_BUTTON1_CHANNEL_ID_INDEX);
...
void TestCase2(void)
{
	...
	assert(Det_TestLastReportError(DIO_MODULE_ID, DIO_INSTANCE_ID, DIO_GET_VERSION_INFO_SID,
								   DIO_E_PARAM_POINTER));
#endif
}
...
void TestCase3(void)
{
	...
	assert(Det_TestLastReportError(DIO_MODULE_ID, DIO_INSTANCE_ID, DIO_WRITE_CHANNEL_SID,
								   DIO_E_PARAM_INVALID_CHANNEL_ID));

	Dio_ReadChannel(DIO_INVALIDE_CHANNEL_NUM);
	assert(Det_TestLastReportError(DIO_MODULE_ID, DIO_INSTANCE_ID, DIO_READ_CHANNEL_SID,
								   DIO_E_PARAM_INVALID_CHANNEL_ID));

#if (DIO_FLIP_CHANNEL_API == STD_ON)
	Dio_FlipChannel(DIO_INVALIDE_CHANNEL_NUM);
	assert(Det_TestLastReportError(DIO_MODULE_ID, DIO_INSTANCE_ID, DIO_FLIP_CHANNEL_SID,
								   DIO_E_PARAM_INVALID_CHANNEL_ID));
#endif
}
...
void TestCase5(void)
{
	...
	{
		SW1_Value = Dio_ReadChannel(DioConf_BUTTON1_CHANNEL_ID_INDEX);
	}
	...
	{
		SW2_Value = Dio_ReadChannel(DioConf_BUTTON2_CHANNEL_ID_INDEX);
	}
...
void test_Dio_ReadPort(void)
{
	Dio_PortType	  PortId = 5; /* PORTA */
	Dio_PortLevelType PortLevel = 0xAA;

	/* Initialize the port */
	Dio_WritePort(PortId, PortLevel);

	/* Read the port and verify the level */
	Dio_PortLevelType ReadPortLevel = Dio_ReadPort(PortId);
	assert(ReadPortLevel == PortLevel);
}
...
void GPIO_Pins_Init(void)
{
	...
	GPIO_PORTF_DIR_REG = (GPIO_PORTF_DIR_REG & 0xE8) |
						 0x06; /* Configure PF0 & PF1 as input pin and Configure PF1 & PF2 as output pins */
