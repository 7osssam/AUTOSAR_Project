...
#ifndef COMMON_MACROS /*header guard*/
...
/* Check if multiple bits are set to 1 based on mask */
#define IS_MASK_SET(reg, mask)		 ((reg) & (mask))

/* Check if multiple bits are cleared to 0 based on mask */
#define IS_MASK_CLEAR(reg, mask)	 (!((reg) & (mask)))
